...
- 区間内で**レベルが最も高く、同じレベルならスコアが最も高い**時点を代表判定とする。同点なら**最も早い**時点（更新条件が `>` のため）。
  スコアだけで選ばないのは、個人化後のレベル（`PersonalRiskModel.schedulingLevel`）が汎用スコアに対して単調でないため（2026-09-12、レビュー R11）
...
- `assessment` は束ねた中でレベル → スコアの順に最も高いもの、`targetDate` は最も早いもの
