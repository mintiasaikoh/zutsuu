...
**未対応・一部対応**: R09 は「起動直後に予報を取り直してから保存」までで、後からの補完（DB 更新）はしていない。R08 の「追加失敗」は台帳と表示から外すが、ユーザーへの通知はしていない。
「追加で確認すべき事項」（BG 更新の位置取得、保存失敗の rollback、再予約の競合、Watch の要約再送、入力の妥当性、平年値の再現性、LINE 版の地点固定と HTTP タイムアウト、リリース設定、結合テスト、文書の古い数値）は未着手。
