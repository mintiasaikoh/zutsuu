...
## 対応記録（2026-09-12、Claude）

| 項目 | 対応 | コミット |
|---|---|---|
| R01 / R02 / R05 / R08 | 台帳（`NotificationLedger`）つきの突き合わせに改め、配信済みを再送しない・静穏時間の変更で付け替える・同じ朝の起床時通知を重ねない・「次の通知」を実際の保留一覧から作る。権限拒否はホームで伝える | e132b37 |
| R03 | Watch は未確認の列を持ち、iPhone が保存に成功したときだけ返信/ack で消す。再送は起動・到達時・記録直後 | 204e632 |
| R04 / R17 / R18 / R19 | LINE 版: 現在を含む時間帯を「現在」に、時刻は JST 固定、pops を持つ系列から読み欠測は残す、morning.yml と push トリガーを廃止、ナウキャストは両方取れたときだけ | 204e632 |
| R06 / R07 / R09 / R10 / R11 / R12 / R13 / R14 / R15 / R16 / R20 / R21 / R23 | 一括修正（コミット本文に内訳） | 64bad5e |
| R22 | 体調と記録 ID を公開ログから外した | b9f983a |

**第 2 段（同日）**: R09 は取れた予報の要因を後から既存記録へ書き込む（スキーマ変更なし）。R08 は追加失敗をホームで伝える。
「追加で確認すべき事項」も対応: BG 更新で位置が取れなければ直近の座標に倒す／保存失敗は rollback／再予約は 1 本ずつ順に走らせる／
Watch 有効化後は手元の要約を再送／非有限の点は落とし、現在を含まない系列は成功にしない／平年値の bin はヘッダの版と次元を検証し、
生成スクリプトが入力の sha256 と年の連続性をマニフェストに残す（再生成で同一 sha256 を確認）／LINE 版は予報区を環境変数に、
ナウキャストのタイルを緯度経度から計算、雨の区間を実時間で切り、HTTP に 15 秒の上限／Apple Weather のマークを表示／
CI に tsc とアプリの結合テスト（`ZutsuuAppTests` 12 件: 予報→予約→配信→再予約、保存の重複防止と後付け、広告の方針）を追加／RiskEngine に 4 行ヘッダ。
