...
`index + hoursAhead` がオーバーフローして SIGTRAP する（debug / release とも）。
internal 関数であり呼び出し側は常に `series.indices` と 1/3/6 の定数を渡すため到達不能。
修正はしないが、「全域で定義」という性質を確立済みとして下流に持ち越さないこと。
同様に負の `hoursAhead` は 0 ではなく後方差分を計算する（境界ガードであって符号検証ではない）。
